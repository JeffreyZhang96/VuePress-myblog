...
1. GET 参数通过 URL 传递，POST 放在请求体中

2. GET 请求在 URL 中传送的参数是有长度限制的，POST 可以传输更多的数据

3. GET 比 POST 更不安全，因为参数直接暴露在 URL 上，所以不能用来传递敏感信息

4. GET 能被保存在书签里，可以被手动输入
...
8. GET 请求只能进行 URL 编码，而 POST 支持多种编码方式

9. 对参数的数据类型，GET 只接受 ASCII 字符，而 POST 没有限制

10. GET 和 POST 都是 TCP 链接。GET 产生一个 TCP 数据包；而 POST 会产生两个 TCP 数据包
...
- **304** Not Modify 未修改，重定位到浏览器。自从上次请求后，请求的网页未修改过。服务器返回此响应时，不会返回网页内容。如果网页自请求者上次请求后再也没有更改过，您应将服务器配置为返回此响应（称为 If-Modified-Since HTTP 标头）。服务器可以告诉 Googlebot 自从上次抓取后网页没有变更，进而节省带宽和开销。
...
[HPACK 算法](https://www.jianshu.com/p/f44b930cfcac)
